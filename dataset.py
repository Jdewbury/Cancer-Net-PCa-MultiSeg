...
class CancerNetPCa:
    def __init__(self, img_dir, mask_dir, modality='cdis', channel_idx=0, seed=42, batch_size=10, train_split=0.7, test_split=0.15, prostate=False, transform=None):
        np.random.seed(seed)
        ...
        train_dataset = CancerNetPCaDataset(img_path[train_idx], mask_path[:, train_idx], modality, channel_idx, prostate, transform)
        val_dataset = CancerNetPCaDataset(img_path[val_idx], mask_path[:, val_idx], modality, channel_idx, prostate, transform)
        test_dataset = CancerNetPCaDataset(img_path[test_idx], mask_path[:, test_idx], modality, channel_idx, prostate, transform)
...
class CancerNetPCaDataset(Dataset):
        def __init__(self, img_path, mask_path, modality, channel_idx, prostate=False, transform=None):
        ...
        def _prepare_data(self):
            ...
            for img, lesion, prostate in zip(self.img_path, self.mask_path[0], self.mask_path[1]):
                img_np = modality_to_numpy(img, self.modality, self.channel_idx)
