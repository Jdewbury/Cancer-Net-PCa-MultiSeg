...
for model in swinunetr attentionunet segresnet unet; do 
    ...
    CUDA_VISIBLE_DEVICES=1 poetry run python scripts/ensemble.py \
        --experiment_dir "results/benchmark/prostate/dwi-adc/$model/dwi-adc-$model"
done
...
for model in swinunetr attentionunet segresnet unet; do 
    ...
    CUDA_VISIBLE_DEVICES=1 poetry run python scripts/ensemble.py \
        --experiment_dir "results/benchmark/prostate/cdis-dwi/$model/cdis-dwi-$model"
done
...
for model in swinunetr attentionunet segresnet unet; do 
    ...
    CUDA_VISIBLE_DEVICES=1 poetry run python scripts/ensemble.py \
        --experiment_dir "results/benchmark/prostate/cdis-adc/$model/cdis-adc-$model"
done
...
for model in swinunetr attentionunet segresnet unet; do 
    ...
    CUDA_VISIBLE_DEVICES=1 poetry run python scripts/ensemble.py \
        --experiment_dir "results/benchmark/prostate/cdis-dwi-adc/$model/cdis-dwi-adc-$model"
done

echo "Multi-modal prostate ensemble benchmarking completed at: $(date)"